...
registration and authentication subsystems, existing and new local Drupal accounts 
(i.e., those created by the core user module) and accounts created by other 
contributed modules are unaffected by this module.
...
Please note that this module will create accounts for users if they authenticate 
against the configured external source, regardless of whether the site 
administrator has choosen "Visitors can create accounts and no administrator 
approval is required" under "Public registrations" at admin/user/settings. 
Local Drupal accounts are goverened by the this setting as usual.
...
-provides an example of how to use SimpleTest (http://drupal.org/project/simpletest)
 to test new drivers.
...
If a driver connects to an SQL database, it must be of the same type as
the database that your Drupal site uses (for example, both must be mysql
or both must be postgres). This is a limitation of the Drupal 6 database
layer and will be changed in Drupal 7 to allow connections to different 
types of database at one time.

INSTALLATION
------------

Just like any other module: unpack the distribution in your site's module
directory, go to Administer > Site building > Modules, and enable. 
...
ilsauthen_driver_login_validation() allows you to validate form elements that you
add to the user login form with ilsauthen_form_alter_driver().
...
A sample test using the SimpleTest module (http://drupal.org/project/simpletest)
is provided to assist driver developers. See tests/ilsaluthen.test for details.
...
drivers will use some sort of web service to connect to authentication services, 
you should consider using the standard technologies that other web clients and 
servers use, such as HTTPS, restricting access to your external service to 
localhost (where possible) or to a specific IP address, etc.
...
you have this module enabled, and you have enabled left the sample driver enabled), 
you may want to change the sample password. If you enable another driver, this 
is not an issue. DO NOT DELETE sample.inc.
...
-This module only populates the username (name), password (pas), and, 
...
-Authenticating against external SQL databases using Drupal's core db
 functions only works if the external database is from the same vendor
 as your local Drupal database (e.g., mysql and mysql, pgsql and pgsql,
 etc.). This is a limitation of Drupal's db layer and will be fixed
 in Drupal 7.
...
-When an external user requests a new password, the core message "Further
 ...
 the active driver's password request message. In Drupal 6 there is no way 
 to programmatically override a message. http://drupal.org/node/127262 and 
 http://drupal.org/node/234320 are issues proposing a fix in Drupal 7.
